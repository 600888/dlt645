...
namespace dlt645 {
    namespace protocol {
        ...
        std::vector<uint8_t> Frame::decodeData(std::span<const uint8_t> data)
        {
            ...
            std::ranges::transform(data, result.begin(), [](uint8_t b) { return b - 0x33; });
            return result;
        }
        ...
        {
            // 基础校验
            if (raw.size() < 12) { // 最小帧长度=起始符(1)+地址(6)+起始符(1)+控制码(1)+数据长度(1)+结束符(1)
                return nullptr;
            }
            ...
            if (startIdx + 7 >= raw.size() || raw[startIdx + 7] != FRAME_START_BYTE) { // 第二个起始符位置校验
                return nullptr;
            }
            ...
            if (dataEnd > raw.size() - 2) { // 预留校验位和结束符
                return nullptr;
            }
